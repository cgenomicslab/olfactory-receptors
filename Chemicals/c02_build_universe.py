"""Build the universe: every odorant and every COCONUT natural product, with descriptors.

Each SMILES is reduced to its largest fragment, rewritten as an RDKit canonical SMILES and
given an InChIKey. COCONUT is deduplicated on that key and molecules on an odorant list are
removed from the background. Then 22 RDKit descriptors, 16 functional groups and a Joback
boiling point are computed for every molecule.

Inputs:  data/coconut_csv-08-2026.csv
         processed_data/odorants_m2or_leffingwell_goodscents.csv
Outputs: results/coconut_parents.parquet, results/universe.parquet
"""
